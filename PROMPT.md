# PlatosWallet.com Landing Page Build

Build a single-page landing page for an AI-powered web design agency using React + Vite + TypeScript + Tailwind CSS. The aesthetic is dark, premium, Apple-inspired with a custom "liquid glass" morphism effect. Pure black background throughout.

The project is already scaffolded with Vite + React + TypeScript. Dependencies installed: tailwindcss, @tailwindcss/vite, hls.js, motion (framer-motion), lucide-react, tailwindcss-animate.

## Setup Tasks
1. Configure Tailwind v4 via `@tailwindcss/vite` plugin in vite.config.ts
2. Set up index.css with Tailwind imports, Google Fonts, CSS variables, and liquid glass styles
3. Configure custom fonts in Tailwind (heading + body families)
4. Build all sections in a single App.tsx or with component files
...
### .liquid-glass-strong:
Same as .liquid-glass but with:
- backdrop-filter: blur(50px)
- box-shadow: 4px 4px 4px rgba(0,0,0,0.05), inset 0 1px 1px rgba(255,255,255,0.15)
- Gradient opacity: 0.5 / 0.2 instead of 0.45 / 0.15

## SECTION 1 — NAVBAR (fixed)
Fixed at top-4, full-width, z-50. Left: logo text "PlatosWallet" in font-heading italic. Center: a liquid-glass rounded-full pill containing nav links ("Home", "Services", "Work", "Process", "Pricing") as text-sm font-medium text-foreground/90 + a solid white bg-white text-black rounded-full "Get Started" button with ArrowUpRight icon.
...
Overlays:
- absolute inset-0 bg-black/5 z-0
...
- Heading: BlurText animation component — text: "The Website Your Brand Deserves" — text-6xl md:text-7xl lg:text-[5.5rem] font-heading italic text-foreground leading-[0.8] tracking-[-4px] — animates word-by-word from bottom with blur-to-clear effect (delay 100ms per word)
...
### BlurText component:
Uses motion/react (framer-motion). Splits text by words, each word animates via IntersectionObserver with filter: blur(10px) → blur(5px) → blur(0px), opacity: 0 → 0.5 → 1, y: 50 → -5 → 0. Step duration 0.35s.
...
- Use hls.js library. Absolute, full cover, z-0. Top + bottom fade gradients (200px each, black to transparent).
...
- Image: placeholder gradient in liquid-glass rounded-2xl overflow-hidden container
...
Cards:
1. Zap icon — "Days, Not Months" — "Concept to launch at a pace that redefines fast."
2. Palette icon — "Obsessively Crafted" — "Every detail considered. Every element refined."
3. BarChart3 icon — "Built to Convert" — "Layouts informed by data. Decisions backed by performance."
4. Shield icon — "Secure by Default" — "Enterprise-grade protection comes standard."
...
Testimonials:
1. Sarah Chen, CEO Luminary — "A complete rebuild in five days. Five. Days. The site looks like we spent six figures on it."
2. Marcus Webb, Head of Growth Arcline — "Conversions up 4x in the first month. The AI optimization is genuinely magical."
3. Elena Voss, Brand Director Helix — "They didn't just design our site—they understood our brand better than we did."
...
Footer: mt-32 pt-8 border-t border-white/10, copyright "© 2026 PlatosWallet" + links (Privacy, Terms, Contact) in text-white/40 text-xs
...
## IMPORTANT NOTES
- This is Tailwind v4. Use `@import "tailwindcss"` in index.css, NOT the old @tailwind directives.
- Configure @tailwindcss/vite plugin in vite.config.ts
- For custom fonts, use `@theme` block in CSS for Tailwind v4 (not tailwind.config.js)
- Use `@theme { --font-heading: 'Instrument Serif', serif; --font-body: 'Barlow', sans-serif; }` in index.css
- Keep components clean and well-organized (separate component files preferred)
- Make sure the build compiles without errors
